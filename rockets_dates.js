...
https://docs.informatica.com/es_es/master-data-management/multidomain-mdm/10-3/guia-de-servicios-de-entidad-de-negocio/llamadas-del-servicio-de-entidad-de-negocio-de-transferencia-de-/formatos-de-fecha-y-hora-utc.html
...
Fecha más horas, minutos, segundos y segundos fraccionarios	YYYY-MM-DDThh:mm:ss.sTZD	1997-07-16T19:20:30.45+01:00
...
function parseUTCStringToMonth(fechaHora){
...
function daysBetweenLaunches(lanz1, lanz2){

  const rocket1 = new Date(lanz1.date_utc)
  const rocket2 = new Date(lanz2.date_utc)

  const diffDays = rocket1.getUTCDate() - rocket2.getUTCDate()

  return diffDays
    
}

console.log(parseUTCStringToMonth())



function getMonthWithMostLaunchFailures(array){
    ...
    array.forEach((elem) => {
      if (elem.success === false){
        dat = new Date(elem.date_utc)
        mesesFallidos.push(dat.getUTCMonth())

      }
    })

    const mes = getElemMaxRep(mesesFallidos)
    
    const fecha = new Date()
    fecha.setMonth(mes)
    const nombreMes = parseUTCStringToMonth(fecha)

    return nombreMes
}


function getElemMaxRep(array){
